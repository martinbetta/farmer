...
doc = Nokogiri::HTML(URI.open('https://fruitbrothers.com.au/product-category/fruit/'))
doc.css('.woocommerce-loop-product__title').each do |title|
  title = title.split.join('-')

  result = Nokogiri::HTML(URI.open(`https://fruitbrothers.com.au/product/#{title}/`))
  result.css('.woocommerce-product-details__short-description p').each do |description|

    photos_url = "https://fruitbrothers.com.au/wp-content/uploads/2019/04/granny-smith-apples-1.jpg"
    result.css(`.woocommerce-product-gallery__wrapper div[data-thumb=#{photos_url}]["content"] `).each do |photo_url|

      result.css(`.attribute_pa_unit-or-measurement option[value="per-kilo"]["content"]`).each do |unit|
...
doc = Nokogiri::HTML(URI.open('https://thefarmeronline.com/category/vegetables'))
doc.css('.mainpro a').each do |title|
  title = title.split.join('-')

  result = Nokogiri::HTML(URI.open(`https://thefarmeronline.com/product/#{title}`))
  result.css('.description p').each do |description|

    result.css(`.#{title}`).each do |photo_url|

      result.css(`.prices .prodetl span`).each do |unit|
...
doc = Nokogiri::HTML(URI.open('https://thefarmeronline.com/category/bakery-confectionery'))
doc.css('.mainpro a').each do |title|
  title = title.split.join('-')
...
doc = Nokogiri::HTML(URI.open('https://thefarmeronline.com/category/beverages'))
doc.css('.mainpro a').each do |title|
  title = title.split.join('-')
...
# snack and instant food

doc = Nokogiri::HTML(URI.open('https://thefarmeronline.com/category/snacks'))
doc.css('.mainpro a').each do |title|
  title = title.split.join('-')
  result = Nokogiri::HTML(URI.open(`https://thefarmeronline.com/product/#{title}`))
  result.css('.description p').each do |description|
